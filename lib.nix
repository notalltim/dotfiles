{ pkgs }: rec {
  ...
  inherit (pkgs.nixgl.auto) nixVulkanNvidia nixGLNvidia nixGLNvidiaBumblebee;

  writeNixGLWrapper = runner: package: stdenvNoCC.mkDerivation
    rec {
      name = package.name;
      installPhase = ''
        runHook preInstall
  ...
  writeIntelGLWrapper = writeNixGLWrapper nixGLIntel;
  ...
  createLuaPlugin = { package, dependencies ? [ ], configs ? "", optional ? false }:
    let
      plugin = {
        type = "lua";
        config = configs;
